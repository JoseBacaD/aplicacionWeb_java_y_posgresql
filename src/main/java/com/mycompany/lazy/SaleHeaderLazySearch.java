 ...
 * @author jerry
 */
public class SaleHeaderLazySearch extends LazyDataModel<Object>{
    private final Object [] params;
    private List<Object> liObject;
    private final String nameSP;
    private final EntityExt entity;
    
    public SaleHeaderLazySearch(EntityExt entity, String nameSP, Object... params) {
     this.params = params;
     this.nameSP = nameSP;
     this.entity = entity;
    }
    ...
    public List<Object> load(int first, int pageSize, Map<String, SortMeta> sortMeta, Map<String, FilterMeta> filterMeta) {
      ...
      if (getRowCount() <= 0){
            if(liObject.size() > 0){
                SaleHeader saleHeader = (SaleHeader)liObject.get(0); 
                setRowCount(saleHeader.getCountRows().intValue());
        }
        }
