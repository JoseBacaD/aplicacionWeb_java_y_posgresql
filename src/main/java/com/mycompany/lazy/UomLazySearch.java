 ...
 * @author jerry
 */
public class UomLazySearch extends LazyDataModel<Object> {
    ...
    public UomLazySearch(EntityExt entity, String nameSP, String nameSPCount,Object... params) {
        this.params = params;
        this.nameSP = nameSP;
    ...
    public List<Object> load(int first, int pageSize, Map<String, SortMeta> sortMeta, Map<String, FilterMeta> filterMeta) {
      ...
      if (getRowCount() <= 0){
            if(liObject.size() > 0){
                UnitOfMeasureUom uom = (UnitOfMeasureUom)liObject.get(0); 
                setRowCount(uom.getCountRows().intValue());
        }
        }
