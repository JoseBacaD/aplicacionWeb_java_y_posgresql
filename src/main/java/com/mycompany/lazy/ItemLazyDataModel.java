/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.mycompany.lazy;

import com.mycompany.dao.BasicDAO;
import com.mycompany.entity.Item;
import com.mycompany.extended.EntityExt;
import java.util.List;
import java.util.Map;
import org.primefaces.model.FilterMeta;
import org.primefaces.model.LazyDataModel;
import org.primefaces.model.SortMeta;

/**
 *
 * @author yo
 */
public class ItemLazyDataModel extends LazyDataModel<Item>{
    private static final long serialVersionUID = 1L;
    
    private List<Item> datasource;
    
    private final Object [] params;
    private List<Object> liObject;
    private final String nameSP;
    
    public ItemLazyDataModel(String nameSP, Object... params) {
     this.params = params;
     this.nameSP = nameSP;
    
    }

    @Override
    public List<Item> load(int first, int pageSize, Map<String, SortMeta> sortBy, Map<String, FilterMeta> filterBy) {
         BasicDAO basicDao = new BasicDAO();
        liObject = basicDao.readSPLazyList(nameSP, first, pageSize, params);
        
       if (getRowCount() <= 0){
            if(liObject.size() > 0){
                Item item = (Item)liObject.get(0); 
                setRowCount(item.getCountRows().intValue());
        }
        }
        
        setPageSize(pageSize);
        return (List<Item>)(Object)liObject;
    }

    @Override
    public String getRowKey(Item item) {
        return String.valueOf(item.getIdItem());
    }

   
   
   

    @Override
    public Item getRowData(String rowKey) {
        for (Item item : (List<Item>)(Object)liObject ) {
            if (item.getIdItem()== Integer.parseInt(rowKey)) {
                return item;
            }
        }

        return null;
    }
    
    

}
